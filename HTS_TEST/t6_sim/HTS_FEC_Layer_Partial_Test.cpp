// HTS_FEC_Layer_Partial_Test.cpp — Step C-3 REDO: Partial barrage × LPI (Layer_Test 기반 확장)
//
// [테스트 방법론]
...
/// 매 trial 별 LPI 변동 파라미터 (coverage < 100% 일 때만 사용; 100% 는 원본 Layer_Test 와 회귀)
struct LPI_Trial_Params {
    int16_t amp{};
    uint32_t info_seed{};
    int32_t timing_offset_chip{};
};
[[nodiscard]] static LPI_Trial_Params make_lpi_params(uint32_t trial_seed) noexcept {
    LPI_Trial_Params p{};
...
// ================================================================
//  Layer 2+3 확장: Partial coverage + trial 단위 LPI (coverage 100% = 원본과 회귀)
// ================================================================
static LayerResult test_fec_direct_partial_lpi(
    int nc, int bps, double js_db, double coverage_pct, int max_rounds,
    int trials, uint32_t seed_base) noexcept {
    FEC_HARQ::Set_IR_Erasure_Enabled(false);
    const int nsym =
        (nc == 64) ? FEC_HARQ::nsym_for_bps(bps) : FEC_HARQ::NSYM16;
    const int total_chips = nsym * nc;
    ...
    for (int t = 0; t < trials; ++t) {
        const uint32_t ts = seed_base ^ static_cast<uint32_t>(t * 0x9E3779B9u);
        const uint32_t il = 0xA5A5A5A5u ^ ts;
        int16_t trial_amp = kAmp;
        uint32_t info_seed = ts;
        if (!full_cov) {
            const LPI_Trial_Params lpi = make_lpi_params(ts);
            trial_amp = lpi.amp;
...
// ============================================================
//  V4 확장: Partial coverage + trial 단위 LPI + V4 전용 timing offset
// ============================================================
static LayerResult test_full_stack_v4_partial_lpi(
    PayloadMode mode, double js_db, double coverage_pct, int max_feeds,
    int trials, int target_bps, uint32_t seed_base) noexcept {
    FEC_HARQ::Set_IR_Erasure_Enabled(false);
    ...
    for (int t = 0; t < trials; ++t) {
        if (mode == PayloadMode::DATA && trials == kTrialsV464) {
            std::printf("  V4-64-partial J/S=%.0f cov=%.0f%%  %d/%d\r", js_db,
                        coverage_pct, t + 1, trials);
            std::fflush(stdout);
        }
        g_last = DecodedPacket{};
        const uint32_t ds = seed_base ^ static_cast<uint32_t>(t * 0x9E3779B9u);
        ...
        if (full_cov) {
            ...
            const LPI_Trial_Params lpi = make_lpi_params(ds);
            trial_amp = lpi.amp;
            timing_off = lpi.timing_offset_chip;
        ...
        for (int rv = 0; rv < max_feeds && success == 0; ++rv) {
            ...
            if (!full_cov && (mode == PayloadMode::DATA) && (nc == 64) &&
                (timing_off != 0)) {
...
int main() {
    ...
    static constexpr double js_sweep[] = {-1, 0,  5,  10, 15, 20,
                                          25, 30, 35, 40, 45, 50};
    static constexpr double coverage_sweep[] = {50.0, 70.0, 100.0};
#if defined(HTS_FEC_POLAR_ENABLE) && !defined(HTS_FEC_POLAR_DISABLE)
    HTS_DWT::Init();
#endif
    std::printf(
...
#endif
    std::printf("  Step C-3 REDO: Partial barrage x LPI (Layer 2+3 + V4-64)\n");
    std::printf(
        "================================================================\n");
    if (!test_layer0()) {
        std::printf("\n[FATAL] Layer 0 FAIL\n");
    ...
    std::printf(
        "\n================================================================\n");
    std::printf("  Layer 2+3: FEC 직접 — Coverage x LPI (100%% = Layer_Test 동일 "
                "seed 회귀)\n");
    std::printf(
        "================================================================\n");
    std::printf("\n── 64chip BPS=4 (Coverage sweep x LPI 변동) ──\n");
    for (double cov : coverage_sweep) {
        std::printf("\n[Coverage %.0f%%]\n", cov);
        for (double js : js_sweep) {
            const uint32_t base_js =
                kSeed ^ static_cast<uint32_t>(static_cast<int>(js * 100));
            const uint32_t cell_seed =
                base_js ^ ((cov >= 100.0 - 1e-9)
                               ? 0u
                               : static_cast<uint32_t>(
                                     static_cast<int>(cov * 7.0)));
            print_row("FEC", test_fec_direct_partial_lpi(
                                  64, 4, js, cov, kMaxRounds, kTrials, cell_seed));
            std::fflush(stdout);
        }
    }
    ...
    std::printf(
        "\n================================================================\n");
    std::printf("  V4 64chip DATA — Coverage x LPI (timing offset DATA only)\n");
    std::printf(
        "================================================================\n");
            ...
            const uint32_t cell_seed =
                base_js ^ ((cov >= 100.0 - 1e-9)
                               ...
                               : static_cast<uint32_t>(
                                     static_cast<int>(cov * 7.0)));
            print_row("V4-64",
                      test_full_stack_v4_partial_lpi(
                          PayloadMode::DATA, js, cov, kMaxRounds, kTrialsV464, 4,
                          cell_seed));
            std::fflush(stdout);
        }
    }
