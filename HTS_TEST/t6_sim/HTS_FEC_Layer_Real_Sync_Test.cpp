// HTS_FEC_Layer_Real_Sync_Test.cpp — Step C-4b Phase 4-Pre: 실제 V4 sync 경로 + LPI
// Phase 4-Pre-3 격리 exe: cl ... /DHTS_REAL_SYNC_LPI_ISO /FeHTS_FEC_Layer_Real_Sync_Test_LPIiso.exe ...
// Phase 4-Pre-4 DIAG exe: cl ... /DHTS_DIAG_TRY_DECODE /FeHTS_FEC_Layer_Real_Sync_Test_DIAG.exe ...
//
// [테스트 방법론]
//   LPI 격리 파라미터(amp/timing/barrage)는 Phase 1 iso 와 동일.
//   V4 경로만 Inject_Payload_Phase 대신 Build_Packet + 전체 Feed_Chip (WAIT_SYNC 경유).
//
...
using ProtectedEngineLocal::RxPhase;
...
/// Step C-4b Phase 4-Pre-3: TX/RX 동일 Holo LPI 시드 (Enable_Holo_LPI 전용)
static constexpr uint32_t kLabHoloLpiSeed[4] = {
    0xC4B40101u, 0xC4B40202u, 0xC4B40303u, 0xC4B40404u};
...
struct LayerResult {
    ...
    double avg_rounds;
    int max_rounds;
    /// Phase 4-Pre-2: `test_full_stack_v4_lpi_real_sync` 전용 (그 외 0 / false)
    bool diag_valid{false};
    uint32_t diag_sync_ok{0};
    uint32_t diag_header_ok{0};
    uint32_t diag_payload_ok{0};
};
...
static void print_row_v4_phase_diag(const LayerResult &r) noexcept {
    if (!r.diag_valid || r.trials <= 0)
        return;
    const int n = r.trials;
    const double ps =
        (n > 0) ? 100.0 * static_cast<double>(r.diag_sync_ok) / static_cast<double>(n)
                : 0.0;
    const double ph =
        (n > 0) ? 100.0 * static_cast<double>(r.diag_header_ok) / static_cast<double>(n)
                : 0.0;
    const double pp =
        (n > 0) ? 100.0 * static_cast<double>(r.diag_payload_ok) / static_cast<double>(n)
                : 0.0;
    std::printf(
        "         sync=%.1f%%  header=%.1f%%  payload=%.1f%%  (%u/%u / %u/%u / "
        "%u/%u)\n",
        ps, ph, pp, r.diag_sync_ok, static_cast<unsigned>(n), r.diag_header_ok,
        static_cast<unsigned>(n), r.diag_payload_ok, static_cast<unsigned>(n));
}
...
// ============================================================
//  V4: LPI + 실제 sync 경로 (Build_Packet → Feed_Chip 전체, WAIT_SYNC 경유)
//  — test_full_stack_v3 와 동일 TX/RX 디스패처 계약, LPI 는 페이로드 구간만 변형
// ============================================================
static LayerResult test_full_stack_v4_lpi_real_sync(
    ...
    static constexpr int kPreambleReps = 8;
    static constexpr int kPreambleBoost = 1;
    /// HTS_Harq_Matrix_Test / Block_Message 과 동일: 전체 패킷 Feed 전후 0칩 가드
    static constexpr int kFeedGuard = 256;
    static constexpr int kMaxC = 256 + (FEC_HARQ::NSYM64 + 12) * 64;
    const int pay_off = (kPreambleReps + 1) * 64 + 128;

    std::vector<int16_t> tx_full_I(static_cast<size_t>(kMaxC));
    std::vector<int16_t> tx_full_Q(static_cast<size_t>(kMaxC));
    std::vector<int16_t> retx_I(static_cast<size_t>(kMaxC));
    std::vector<int16_t> retx_Q(static_cast<size_t>(kMaxC));
    std::vector<int16_t> wrk_I(static_cast<size_t>(kMaxC));
    std::vector<int16_t> wrk_Q(static_cast<size_t>(kMaxC));
    std::vector<int16_t> pay_I(static_cast<size_t>(total_chips));
    std::vector<int16_t> pay_Q(static_cast<size_t>(total_chips));
    ...
    uint32_t n_sync_milestone = 0u;
    uint32_t n_header_milestone = 0u;
    uint32_t n_payload_milestone = 0u;

    auto apply_rx_bps = [&](HTS_V400_Dispatcher& d) noexcept {
        if (mode == PayloadMode::DATA) {
            uint32_t nf = 100u;
            switch (target_bps) {
            case 3: nf = 3000u; break;
            case 4: nf = 1000u; break;
            case 5: nf = 300u; break;
            default: nf = 100u; break;
            }
            d.Update_Adaptive_BPS(nf);
        }
    };
    ...
    for (int t = 0; t < trials; ++t) {
        if (mode == PayloadMode::DATA && trials == kTrialsV464) {
            std::printf(
                ...
                holo_lpi_engine_enabled ? "ON" : "OFF", js_db, coverage_pct,
                lpi_mode_name(lpi_mode), t + 1, trials);
            std::fflush(stdout);
        }
        g_last = DecodedPacket{};
        const uint32_t ds = seed_base ^ static_cast<uint32_t>(t * 0x9E3779B9u);
        ...
        if (holo_lpi_engine_enabled) {
            ...
            tx_disp.Disable_Holo_LPI();
        }

        const int pkt_chips = tx_disp.Build_Packet(
            mode, info, 8, trial_amp, tx_full_I.data(), tx_full_Q.data(), kMaxC);
        if (pkt_chips <= 0)
            continue;
        if (pay_off + total_chips > pkt_chips)
            continue;

        const int retx_chips = tx_disp.Build_Retx(
            mode, info, 8, trial_amp, retx_I.data(), retx_Q.data(), kMaxC);
        if (retx_chips <= 0)
            continue;
...
#endif
        if (holo_lpi_engine_enabled) {
            rx_disp.Enable_Holo_LPI(kLabHoloLpiSeed);
        } else {
            rx_disp.Disable_Holo_LPI();
        }

        bool hit_read_hdr_or_pay = false;
        bool hit_read_payload = false;
        auto note_phase = [&](RxPhase ph) noexcept {
            if (ph == RxPhase::READ_HEADER || ph == RxPhase::READ_PAYLOAD)
                hit_read_hdr_or_pay = true;
            if (ph == RxPhase::READ_PAYLOAD)
                hit_read_payload = true;
        };
        ...
        for (int feed = 0; feed < max_feeds && success == 0; ++feed) {
            ...
            if (feed == 0 || !rx_disp.Is_Retx_Ready()) {
                ...
                std::memcpy(wrk_I.data(), tx_full_I.data(),
                              static_cast<size_t>(pkt_chips) * sizeof(int16_t));
                std::memcpy(wrk_Q.data(), tx_full_Q.data(),
                              static_cast<size_t>(pkt_chips) * sizeof(int16_t));
                for (int i = 0; i < total_chips; ++i) {
                    pay_I[static_cast<size_t>(i)] =
                        wrk_I[static_cast<size_t>(pay_off + i)];
                    pay_Q[static_cast<size_t>(i)] =
                        wrk_Q[static_cast<size_t>(pay_off + i)];
                }
                const int16_t* pI = pay_I.data();
                const int16_t* pQ = pay_Q.data();
                if ((mode == PayloadMode::DATA) && (nc == 64) &&
                    (timing_off != 0)) {
                    apply_timing_offset_chips(pay_I.data(), shI.data(),
                                              total_chips, timing_off);
                    apply_timing_offset_chips(pay_Q.data(), shQ.data(),
                                              total_chips, timing_off);
                    pI = shI.data();
                    pQ = shQ.data();
                }
                if (js_db >= 0.0) {
                    if (full_cov) {
                        add_barrage(pI, dbl.data(), total_chips, js_db, rng);
                    } else {
                        add_barrage_partial(pI, dbl.data(), total_chips, js_db,
                                            coverage_pct, rng);
                    }
                    agc_quantize(dbl.data(), rxI.data(), rxQ.data(),
                                 total_chips);
                } else {
                    std::memcpy(rxI.data(), pI,
                                static_cast<size_t>(total_chips) *
                                    sizeof(int16_t));
                    std::memcpy(rxQ.data(), pQ,
                                static_cast<size_t>(total_chips) *
                                    sizeof(int16_t));
                }
                for (int i = 0; i < total_chips; ++i) {
                ...
                const int16_t* rI = retx_I.data();
                const int16_t* rQ = retx_Q.data();
                if ((mode == PayloadMode::DATA) && (nc == 64) &&
                    (timing_off != 0)) {
                    apply_timing_offset_chips(retx_I.data(), shI.data(),
                                              retx_chips, timing_off);
                    apply_timing_offset_chips(retx_Q.data(), shQ.data(),
                                              retx_chips, timing_off);
                    rI = shI.data();
                    rQ = shQ.data();
                }
                if (js_db >= 0.0) {
                    if (full_cov) {
                        add_barrage(rI, dbl.data(), retx_chips, js_db, rng);
                    } else {
                        add_barrage_partial(rI, dbl.data(), retx_chips, js_db,
                                            coverage_pct, rng);
                    }
                    agc_quantize(dbl.data(), rxI.data(), rxQ.data(),
                                 retx_chips);
                } else {
                    std::memcpy(rxI.data(), rI,
                                static_cast<size_t>(retx_chips) *
                                    sizeof(int16_t));
                    std::memcpy(rxQ.data(), rQ,
                                static_cast<size_t>(retx_chips) *
                                    sizeof(int16_t));
                }
                src_I = rxI.data();
            ...
            if (use_retx) {
                for (int i = 0; i < src_n; ++i) {
                    rx_disp.Feed_Retx_Chip(
                        src_I[i], src_Q[i]);
                    note_phase(rx_disp.Get_Phase());
                }
            } else {
                for (int g = 0; g < kFeedGuard; ++g) {
                    rx_disp.Feed_Chip(0, 0);
                    note_phase(rx_disp.Get_Phase());
                }
                for (int i = 0; i < src_n; ++i) {
                    rx_disp.Feed_Chip(src_I[i], src_Q[i]);
                    note_phase(rx_disp.Get_Phase());
                }
                for (int g = 0; g < kFeedGuard; ++g) {
                    rx_disp.Feed_Chip(0, 0);
    ...
    if (res.crc_ok > 0)
        res.avg_rounds /= res.crc_ok;
    res.diag_valid = true;
    res.diag_sync_ok = n_sync_milestone;
    res.diag_header_ok = n_header_milestone;
    res.diag_payload_ok = n_payload_milestone;
    return res;
}
} // namespace
int main() {
...
#  endif
    ...
    run_diag_cell("=== DIAG NONE -1 dB (baseline) ===", LPI_Mode::NONE, -1.0);
    run_diag_cell("=== DIAG TIM -1 dB (HARQ spin-up) ===",
                  LPI_Mode::TIMING_ONLY, -1.0);
    ...
    static constexpr double js_sweep[] = {-1.0, 0.0, 5.0, 10.0};
...
#  if defined(HTS_FEC_POLAR_ENABLE) && !defined(HTS_FEC_POLAR_DISABLE)
    HTS_DWT::Init();
#  endif
#  if defined(HTS_REAL_SYNC_LPI_ISO)
    std::printf(
        "\n================================================================\n");
#if defined(HTS_FEC_POLAR_ENABLE) && !defined(HTS_FEC_POLAR_DISABLE)
...
#endif
    std::printf(
        "  Step C-4b Phase 4-Pre-3: Holo LPI 엔진 ON/OFF 격리 (TIM/BOTH, "
        "Cov50%%, J/S=-1/0/5)\n");
    std::printf(
        "================================================================\n");
    if (!test_layer0()) {
    ...
    static constexpr double js_iso[] = {-1.0, 0.0, 5.0};
    static constexpr double cov_iso[] = {50.0};
    static constexpr LPI_Mode lpi_iso[] = {LPI_Mode::TIMING_ONLY,
                                           LPI_Mode::BOTH};
    for (const bool holo_on : {true, false}) {
        std::printf("\n== V4-64 Holo LPI %s (실 sync, Cov 50%%) ==\n",
                    holo_on ? "ON" : "OFF");
        for (LPI_Mode lm : lpi_iso) {
            for (double cov : cov_iso) {
                std::printf("\n[Holo=%s LPI=%s, Cov=%.0f%%]\n",
                            holo_on ? "ON" : "OFF", lpi_mode_name(lm), cov);
                for (double js : js_iso) {
                    const uint32_t cell_seed = iso_cell_seed(kSeed, js, cov, lm);
                    const LayerResult rv = test_full_stack_v4_lpi_real_sync(
                        PayloadMode::DATA, js, cov, lm, kMaxRounds, kTrialsV464,
                        4, cell_seed, holo_on);
                    print_row("V4-64-Sync", rv);
                    print_row_v4_phase_diag(rv);
                    std::fflush(stdout);
                }
            }
    ...
    std::printf("\n=== 완료 (Step C-4b Phase 4-Pre-3 Holo LPI 격리) ===\n");
    return 0;
#  else
    std::printf(
        "\n================================================================\n");
    ...
    std::printf("  [FEC] Conv K=7 + REP4 (HTS_FEC_POLAR_DISABLE)\n");
#endif
    std::printf("  Step C-4b Phase 4-Pre-2: V4 실 sync + 단계별 진단 (Cov50%%)\n");
    std::printf(
        "================================================================\n");
    if (!test_layer0()) {
        std::printf("\n[FATAL] Layer 0 FAIL\n");
    ...
    std::printf("\n== V4 64chip DATA LPI (실제 sync 경로, Cov 50%%만) ==\n");
#if defined(HTS_FEC_POLAR_ENABLE) && !defined(HTS_FEC_POLAR_DISABLE)
    HTS_DWT::Stats_Reset();
#endif
    static constexpr double cov_v4_pre2[] = {50.0};
    for (LPI_Mode lm : lpi_modes) {
        for (double cov : cov_v4_pre2) {
            std::printf("\n[LPI=%s, Cov=%.0f%%]\n", lpi_mode_name(lm), cov);
            for (double js : js_sweep) {
                const uint32_t cell_seed =
                    iso_cell_seed(kSeed, js, cov, lm);
                const LayerResult rv = test_full_stack_v4_lpi_real_sync(
                    PayloadMode::DATA, js, cov, lm, kMaxRounds, kTrialsV464, 4,
                    cell_seed);
                print_row("V4-64-Sync", rv);
                print_row_v4_phase_diag(rv);
                std::fflush(stdout);
