...
class HTS_Polar_Codec {
  public:
    // ── 코드 파라미터 ──────────────────────────────────────────
    static constexpr int N = 512;        ///< 코드 길이 (2^9, HTS 시스템 최적)
    static constexpr int LOG_N = 9;      ///< log2(N)
    static constexpr int K = 80;         ///< 정보 비트 (64데이터 + 16CRC)
    static constexpr int FROZEN = N - K; ///< 432 frozen 비트
    static constexpr int DATA_BYTES = 8; ///< 데이터 바이트 수
    static constexpr int CRC_BITS = 16;  ///< CRC-16 비트
    // ── SCL 파라미터 ──────────────────────────────────────────
    static constexpr int SCL_L =
        8; ///< 리스트 크기 (양산 primary, L=16은 margin)
    // ── 인코딩 ────────────────────────────────────────────────
...
// ── 메모리 예산 정적 검증 ────────────────────────────────────
static_assert(HTS_Polar_Codec::N == 512, "Polar N must be 512");
static_assert(HTS_Polar_Codec::K == 80, "Polar K must be 80");
static_assert(HTS_Polar_Codec::FROZEN == 432, "Frozen count mismatch");
static_assert(HTS_Polar_Codec::LOG_N == 9, "LOG_N mismatch");
} // namespace ProtectedEngine
