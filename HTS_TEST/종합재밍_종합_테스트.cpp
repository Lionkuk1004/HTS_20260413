// =========================================================================
// 종합재밍_종합_테스트.cpp — HTS B-CDMA 조합 재밍 생존성 (PC 전용)
//
//  목적
//  ----
//  Tensor FEC + 인터리버 + (가상) LTE HARQ 를 한 파이프라인으로 묶고,
//  AWGN / 바라지 / CW / EMP 네 채널을 같은 강도 스윕으로 비교한다.
//
//  데이터 경로 (블록 단위)
//  ----------------------
...
//
//  빌드: HTS_검증_종합재밍.vcxproj — 소스는 본 cpp 하나 + HTS_LIM 링크
// =========================================================================
...
void print_banner() {
    std::cout << "\n";
    print_line('=', 100);
    std::cout << "  [조합 재밍 종합] Tensor FEC + 인터리버 + HARQ\n";
    std::cout << "  채널: AWGN | 바라지 | CW | EMP  ×  강도 10단계  →  한 표로 요약\n";
    print_line('=', 100);
    std::cout << "\n";
}
