...
//
//  [정렬] HTS_V400_Dispatcher.cpp:
//  g_harq_ccm_union·g_sic_exp_*·k_walsh_dummy_iq_
...
enum class RxPhase : uint8_t {
    ...
    RF_SETTLING =
        3u ///< RF PLL 안정 구간 — 칩 송수신 Blanking(타이머는 Feed_Chip 경유)
};
...
struct DecodedPacket {
    ...
    /// CRC/디코드 성공: DECODE_MASK_OK, 실패: DECODE_MASK_FAIL (호출부 & 마스크
    /// 결합)
...
class HTS_V400_Dispatcher {
  public:
    ...
    /// @brief 소멸자 — CCM·버퍼·시드·ajc_ 스토리지 개별 secureWipe (this 통째
    /// wipe 없음)
    ~HTS_V400_Dispatcher() noexcept;
    ...
    int Build_Retx(PayloadMode mode, const uint8_t *info, int ilen, int16_t amp,
    ...
    /// @brief PC·시험 하네스 전용: NF 간접 경로 없이 64칩 DATA BPS(3~6) 직접
    /// 설정.
    /// @note `FEC_HARQ::bps_clamp_runtime` 적용(예: M4 RAM 레이아웃이면
    /// BPS<4→4).
    ///       운용 단일 진실은 `Update_Adaptive_BPS`+RF — 본 API는 재밍
    ///       스윕·논문 재현용.
    void Set_Lab_BPS64(int bps) noexcept;
    /// @brief 재밍 시험 하네스: IR-HARQ·바라지 가정에 맞춰 I/Q 모드를
    /// IQ_SAME으로 고정.
    ...
    /// @brief IR DATA·64칩·IQ_SAME SIC — CRC 실패 후 재인코딩 예상 칩을 다음
    /// 라운드에서 감산
    ...
    /// @brief SIC용 Walsh 진폭 — TX `Build_Packet(..., amp, ...)` 와 동일
    /// 스케일 권장 (기본 300)
    void Set_SIC_Walsh_Amp(int16_t amp) noexcept;
    /// @brief IR-HARQ 1라운드 RTT (ms) — HTS 독자 링크; 양산 시 실측·스케줄로
    /// 확정.
    /// @note Chase/소프트 텐서 벤치의 `LTE_HARQ_Controller::HARQ_RTT_MS`(8ms)와
    /// 별도.
    ///       PC 벤치 `HTS_Fractal_Channel_Compare` 가 IR 경로 지연 proxy에
    ///       동기화.
    ...
    [[nodiscard]] bool Get_CW_Cancel() const noexcept {
        return cw_cancel_enabled_;
    }
    ...
    void Set_SoftClip_Policy(SoftClipPolicy p) noexcept {
        soft_clip_policy_ = p;
    }
    SoftClipPolicy Get_SoftClip_Policy() const noexcept {
        return soft_clip_policy_;
    }
    ...
    /// @brief FHSS 도약 채널(0~127) — `seed`·`seq` 혼합, `/`·`%` 없음(`&
    /// 0x7F`만)
    [[nodiscard]] static uint8_t FHSS_Derive_Channel(uint32_t seed,
                                                     uint32_t seq) noexcept;
    /// @brief TX 역할 도약: 현재 `tx_seq_`로 채널 산출 후 시퀀스 증가·Blanking
    /// 진입
    /// @return 0~127 정상, 0xFF 이미 RF_SETTLING 또는 전이 실패
    [[nodiscard]] uint8_t FHSS_Request_Hop_As_Tx() noexcept;
    /// @brief RX 역할 도약: 현재 `rx_seq_`로 채널 산출 후 시퀀스 증가·Blanking
    /// 진입
  ...
  private:
    ...
    //
    //  linker script 예시:
    //    .ccm_bss (NOLOAD) : { _sccm_bss = .; *(.ccm_bss) _eccm_bss = .; } >
    //    CCM
    //
    ...
    //
    //  구현: 16슬롯 LUT(4상태) + key 클램프(>=16 → 불법 슬롯), 반환은 풀비트
    //  마스크
