// ============================================================================
// HTS_CFO_Bank_Test_v9_phase.cpp — Step 2C Lab: carrier phase ψ sweep
// v4/v8 동일: 128 Walsh preamble + 64 payload, V5a + Two_Candidates,
// SNR=30 dB, cfo=0 Hz, timing=0.  channel_apply 전에 칩 스트림에
// apply_carrier_phase(ψ) 만 적용. 양산 코드 미변경 (Lab 전용).
// ============================================================================
#include <bit>
#include <cmath>
#include <cstdint>
...
static TrialOutcome run_single_trial_v9(HTS_Holo_Tensor_4D_TX& tx_tensor,
...
static Agg run_phase_psi(HTS_Holo_Tensor_4D_TX& tx_tensor,
    ...
    for (int trial = 0; trial < num_trials; ++trial) {
        const TrialOutcome o =
            run_single_trial_v9(tx_tensor, rx_tensor, snr_db, trial, psi_rad);
        if (!o.encode_ok)
            continue;
        if (o.sync_ok)
            ++a.sync_pass;
...
int main() {
    ...
    std::printf("=== Step 2C Phase offset sweep (cfo=0, timing=0, SNR=%d dB) "
                "===\n",
                kSnrDb);
    std::printf("Lab v9: carrier phase psi on full chip stream; trials=%d/pt\n",
                kTrials);
    ...
    for (int i = 0; i < nP; ++i) {
        ...
        const Agg a = run_phase_psi(tx_tensor, rx_tensor, psi_rad, kSnrDb,
                                    kTrials);
        std::printf("%5.1f | %3d/100 | %3d/100 | %s\n", psi_deg,
                    a.decode_pass, a.sync_pass, row_tag(a.decode_pass));
    }
    std::printf("--------+-----------+-----------+------\n");
