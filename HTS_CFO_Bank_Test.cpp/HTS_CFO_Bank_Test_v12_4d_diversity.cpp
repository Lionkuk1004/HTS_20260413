// ============================================================================
// HTS_CFO_Bank_Test_v12_4d_diversity.cpp — Step 2-3d-2 Lab TX + Step 2-3e A2' RX
// Preamble 동일(v11). Payload: ts_I=lab_tx_seq, ts_Q=lab_tx_seq^1 로 이중
// Encode_Block 후 I/Q 각각 다른 4D 칩(양산 B' 와 동일). RX: metric 합산.
// lab_tx_seq 고정 0u (TX/RX 정합). 양산 코드 미변경 (Lab 전용).
// ============================================================================
#include <bit>
#include <cmath>
#include <cstdint>
...
namespace {
/// 고정 슬롯: TX 이중 Encode 및 RX diversity decode 정합 (영준님 권장 고정값)
constexpr uint32_t kLabBprimeTxSlot = 0u;
...
/// Step 2-3e A2': I/Q 각각 Decode_Block (단일 칩열) + 비트 합 BPTE (Dispatcher 동일).
static uint32_t lab_rx_decode_4d_a2_prime_slots(
    ...
    int8_t* out_sym1) noexcept
{
    ...
    const uint32_t ok_I = rx.Decode_Block(
        corr_I, N_chip, 0xFFFFFFFFFFFFFFFFull, bits_I, K_bits);
    (void)rx.Set_Time_Slot(rx_slot ^ 1u);
    const uint32_t ok_Q = rx.Decode_Block(
        corr_Q, N_chip, 0xFFFFFFFFFFFFFFFFull, bits_Q, K_bits);
    (void)rx.Set_Time_Slot(rx_slot);
...
/// 양산 B' 와 동일: ts_I = kLabBprimeTxSlot, ts_Q = kLabBprimeTxSlot ^ 1
static int tx_build_tensor(HTS_Holo_Tensor_4D_TX& tensor,
                           const int8_t* data_bits, int16_t* tx_I, int16_t* tx_Q,
                           int amp) noexcept {
    int8_t chip_bpsk_I[HOLO_CHIP_COUNT];
    int8_t chip_bpsk_Q[HOLO_CHIP_COUNT];
    (void)tensor.Set_Time_Slot(kLabBprimeTxSlot);
    ...
    (void)tensor.Set_Time_Slot(kLabBprimeTxSlot ^ 1u);
    if (tensor.Encode_Block(
...
static TrialOutcome run_single_trial_v12(HTS_Holo_Tensor_4D_TX& tx_tensor,
    ...
    (void)tx_tensor.Set_Time_Slot(kLabBprimeTxSlot);
    ...
    if (lab_rx_decode_4d_a2_prime_slots(
            rx_tensor, kLabBprimeTxSlot, corr_I, corr_Q,
            static_cast<uint16_t>(ExperimentConfig::kN),
...
static Agg run_phase_psi(HTS_Holo_Tensor_4D_TX& tx_tensor,
    ...
    for (int trial = 0; trial < num_trials; ++trial) {
        const TrialOutcome o =
            run_single_trial_v12(tx_tensor, rx_tensor, snr_db, trial,
                                 psi_rad);
        if (!o.encode_ok)
            continue;
        if (o.sync_ok)
...
int main() {
    ...
    std::printf("=== Step 2-3d-2 Phase sweep, TX 양산 B' 정합 (Lab v12) ===\n");
    std::printf("Preamble: I=W64 row63, Q=row21 (ch0-63); I=+amp, Q=row62 "
                "(ch64-127).\n");
    std::printf("Payload: dual Encode_Block ts_I=0 ts_Q=1, I/Q separate 4D.\n");
    ...
    int16_t pay_I[64], pay_Q[64];
    const int pay_ok =
        tx_build_tensor(tx_tensor, demo_bits, pay_I, pay_Q,
