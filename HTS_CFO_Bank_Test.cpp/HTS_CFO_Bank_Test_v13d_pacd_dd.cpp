// ============================================================================
// HTS_CFO_Bank_Test_v13d_pacd_dd.cpp — Lab v13d: PaCD + DD + 3-way BPTE + brutal
// random sweep (SNR/CFO/timing/MP/psi per trial). Lab 전용, 양산 코드 미변경.
// ============================================================================
#include <bit>
#include <cmath>
#include <cstdint>
...
static TrialDec run_brutal_trial(HTS_Holo_Tensor_4D_TX& tx_tensor,
    ...
    const bool pacd_ok =
        ...
        (pick_best_cand_err(pacd_c0, pacd_c1, data_bits, ExperimentConfig::kK) ==
         0);
    ...
    if (ok_pacd == HTS_Holo_Tensor_4D_RX::SECURE_TRUE) {
        ...
        if (tx_build_tensor(tx_tensor, static_cast<uint32_t>(trial), pacd_c0,
                            recon_pay_I, recon_pay_Q, ExperimentConfig::kAmp) !=
            0) {
    ...
    const bool dd_ok =
        ...
        (pick_best_cand_err(dd_c0, dd_c1, data_bits, ExperimentConfig::kK) == 0);
    ...
    if (ok_leg == HTS_Holo_Tensor_4D_RX::SECURE_TRUE &&
        ...
        const int64_t sum_d =
            metric_abs_sum(metric_dd, ExperimentConfig::kK);

        const int64_t diff_lp = sum_p - sum_l;
        const int32_t mask_pacd_lp =
            static_cast<int32_t>(~(diff_lp >> 63));
        const int32_t mask_legacy_lp =
            static_cast<int32_t>(diff_lp >> 63);
        const int64_t max_lp =
            (sum_p & static_cast<int64_t>(mask_pacd_lp)) |
            (sum_l & static_cast<int64_t>(mask_legacy_lp));

        const int64_t diff_d = sum_d - max_lp;
        const int32_t mask_dd = static_cast<int32_t>(~(diff_d >> 63));
        const int32_t mask_lp_final = static_cast<int32_t>(diff_d >> 63);

        int8_t bits_combined[HOLO_MAX_BLOCK_BITS];
        for (int k = 0; k < ExperimentConfig::kK; ++k) {
            const int8_t bits_lp = static_cast<int8_t>(
                (pacd_c0[k] & static_cast<int8_t>(mask_pacd_lp)) |
                (leg_c0[k] & static_cast<int8_t>(mask_legacy_lp)));
            bits_combined[k] = static_cast<int8_t>(
                (dd_c0[k] & static_cast<int8_t>(mask_dd)) |
                (bits_lp & static_cast<int8_t>(mask_lp_final)));
        }
        combined_ok =
            (bit_err_vs_ref(bits_combined, data_bits, ExperimentConfig::kK) == 0);
    } else if (ok_leg == HTS_Holo_Tensor_4D_RX::SECURE_TRUE &&
               ok_pacd == HTS_Holo_Tensor_4D_RX::SECURE_TRUE) {
        const int64_t sum_l =
            metric_abs_sum(metric_legacy, ExperimentConfig::kK);
        const int64_t sum_p =
            metric_abs_sum(metric_pacd, ExperimentConfig::kK);
        const int64_t diff = sum_p - sum_l;
        const int32_t mask_legacy = static_cast<int32_t>(diff >> 63);
        const int32_t mask_pacd = ~mask_legacy;
        int8_t bits_combined[HOLO_MAX_BLOCK_BITS];
        for (int k = 0; k < ExperimentConfig::kK; ++k) {
            bits_combined[k] = static_cast<int8_t>(
                (pacd_c0[k] & static_cast<int8_t>(mask_pacd)) |
                (leg_c0[k] & static_cast<int8_t>(mask_legacy)));
        }
        combined_ok =
            (bit_err_vs_ref(bits_combined, data_bits, ExperimentConfig::kK) == 0);
    }
...
int main() {
    ...
    std::printf("## Lab v13d Brutal Random Sweep\n\n");
    std::printf("### Bin 별 통계\n\n");
